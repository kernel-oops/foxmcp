...
### Added
- **Native tab groups: `tabs_group`, `tabs_ungroup`, `tab_groups_update`, `tab_groups_query`.** `tabs_group` groups existing tabs and optionally sets a title and colour without activating tabs or focusing windows. If styling a newly created group fails, the group is rolled back rather than left half-styled; if the tabs were added to a group that already existed, that group is left alone and the error names it, so the styling can be retried with `tab_groups_update` rather than by regrouping. `tabs_ungroup` removes tabs from their group. `tab_groups_update` restyles a group's title, colour or collapsed state without touching its tabs. `tab_groups_query` lists groups and their IDs, which is the only way to recover the ID of a group the caller did not just create. Requires Firefox 139+ and the `tabGroups` extension permission; existing tab creation behaviour is unchanged.
